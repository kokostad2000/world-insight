# AC-36 真实 Mac 睡眠验收：历史准备记录

**当前状态：真实睡眠已于 2026-09-22 通过。** 最终结论与实际运行字段见 [真实睡眠验收](mac-sleep-acceptance-20260922.md) 和 [结构化 JSON](mac-sleep-acceptance-20260922.json)。

以下为 2026-09-21 的原始准备与执行入口，保留用于解释隔离基线及当时为何不能判定通过。模拟时钟、SIGKILL和进程锁测试继续单列，不能替代后来完成的真实睡眠。
