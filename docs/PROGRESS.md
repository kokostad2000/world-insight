...
- 2026-09-22真实GKG批次为6,578,603 B压缩、20,389,639 B解压；有界上限调整为8/32 MiB，真实文件离线解析1,549条元数据。正式调度随后以2次新增请求处理7,407,720 B新批次，五任务全部complete；UNGA 4条、其余0条。成功状态残留旧错误码的问题已修复并以缓存重跑验证，请求数未增加，旧124个失败来源版本与1个恢复事件仍在。见 [大小边界与验证](evidence/gkg-live-size-boundary-20260922.md)；成功不代表召回完整或达到M4数量门槛。
