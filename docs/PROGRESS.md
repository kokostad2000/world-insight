...
- AC-26已准备提交`00e5a5a`的中文／空格路径隔离副本、独立数据与8896端口，完成正式初始化和正常停止；两次Finder Computer Use复核仍显示辅助功能/屏幕录制未授权，因此没有伪造双击通过。见`docs/evidence/finder-double-click-preparation-20260921.md`。
- AC-36新增零网络、拒绝非空目录的隔离准备脚本与660秒真实睡眠执行清单；尚未让Mac睡眠，也未把准备状态写成通过。见`docs/evidence/mac-sleep-acceptance-pending.md`。
...
1. AC矩阵已按当前证据刷新；工程M3仍受 AC-26 Finder双击权限和 AC-36实际OS睡眠唤醒阻挡，不能宣告完成。Finder隔离实例已停在可直接继续双击的安全状态。
