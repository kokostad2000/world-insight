...
export const schemas = {
 topics:[f('question','研究问题','textarea',{required:true,full:true,placeholder:'例如：某项政策将如何影响地区贸易？'}),f('regions','地区（逗号或换行分隔）','lines'),f('actors','参与方（逗号或换行分隔）','lines'),f('country_codes','国家背景代码（逗号或换行分隔）','lines',{placeholder:'如 USA、CHN',hint:'按明确国家读取已保存的 WDI 背景指标；观测年份保持不变。'}),f('source_ids','此议题的采集来源','sources',{full:true,hint:'不勾选表示使用默认已启用免费来源；所选来源还须允许采集本议题。'}),f('time_range','研究时间范围','text',{placeholder:'例如：2026 年下半年'}),f('status','议题状态','select',{options:csvOptions(['active','paused','archived'])}),f('keywords','关键词','lines'),f('exclude_keywords','排除词','lines'),f('rationale','关注理由','textarea',{full:true}),f('followed','关注此议题，纳入变化简报','checkbox')],
 ...
 sources:[f('name','来源名称','text',{required:true}),f('adapter','接入方式','select',{options:[['manual','人工链接登记'],['rss','官方 RSS'],['gdelt','GDELT DOC 新闻查询'],['gdelt_gkg','GDELT GKG 批次新闻发现（备用）'],['world_bank','World Bank WDI']]}),f('domain','来源域名'),f('enabled','启用此免费来源','checkbox'),f('languages','语言覆盖','lines'),f('regions','地区覆盖','lines'),f('budget_daily','每日免费请求上限','number',{min:0,required:true}),f('interval_seconds','刷新间隔（秒）','number',{min:60,required:true}),f('retention_days','来源内容保留上限（天）','number',{min:1,hint:'按授权填写；无更短期限留空。到期只保留允许的元数据、用户笔记与引用占位。'}),f('config.feed_url','RSS 地址','url',{full:true,hint:'批量采集只允许已经审核的来源。其他来源仍可人工登记链接。'}),f('config.query','GDELT DOC 查询（GKG 只使用议题关键词）','text',{full:true}),f('config.countries','WDI 国家 ISO3 代码','lines'),f('config.indicators','WDI 白名单指标','lines',{hint:'NY.GDP.MKTP.CD、SP.POP.TOTL'}),f('config.topic_ids','采集关联议题 ID（每行一项）','lines',{full:true}),section('授权与使用范围'),f('license_url','授权依据链接','url',{full:true}),f('checked_at','许可核对日期','date'),f('rights.fetch','允许获取','checkbox'),f('rights.store','允许存储','checkbox'),f('rights.display','允许展示','checkbox'),f('rights.export','允许导出','checkbox'),f('rights.ai','允许用于 AI（功能关闭）','checkbox')],
};
...
export function renderField(field, record, context) {
 ...
 if(field.type==='checkbox')control=`<div class="check-field"><input type="checkbox" ${attrs} ${value?'checked':''}><label for="${esc(id)}">${esc(field.title)}</label></div>`;
   ...
   const options=context.sources.filter(x=>!['paid','ai'].includes(x.adapter)).map(x=>[x.id,x.name]);
   selected.forEach(id=>{if(!options.some(x=>x[0]===id))options.push([id,`已保存的来源 ${id}`]);});
   control=`<div class="refs">${options.map(([v,t])=>`<label><input type="checkbox" name="${esc(field.key)}" value="${esc(v)}" ${selected.includes(v)?'checked':''}><span>${esc(t)}</span></label>`).join('')||'<span class="muted">尚无来源配置</span>'}</div>`;
 }
 else if(['refs','claims'].includes(field.type)){
   const selected=Array.isArray(value)?value:[];
   const options=field.type==='refs'?referenceOptions(context.evidence,selected):field.type==='sources'?context.sources.filter(x=>!['paid','ai'].includes(x.adapter)).map(x=>[x.id,x.name]):context.claims.map(x=>[x.id,`${x.subject}：${x.statement}`]);
   selected.forEach(id=>{if(!options.some(x=>x[0]===id))options.unshift([id,`历史说法 ${id}`]);});
   control=`<div class="reference-control" data-reference-kind="${field.type==='refs'?'evidence':'claims'}" data-reference-field="${esc(field.key)}"><div class="reference-search"><input type="search" aria-label="搜索${esc(field.title)}" placeholder="搜索资料库中的更多记录"><button type="button" data-action="search-references" class="small">搜索</button></div><div class="refs">${options.length?options.map(([v,t])=>`<label><input type="checkbox" name="${esc(field.key)}" value="${esc(v)}" ${selected.includes(v)?'checked':''}><span>${esc(t)}</span></label>`).join(''):'<span class="muted small-text">尚无可引用记录。可搜索资料库或先保存草稿。</span>'}</div><div class="hint reference-search-status">显示近期记录；搜索覆盖整个资料库，已选版本保留。</div></div>`;
...
export function readForm(form,schema) {
 ...
 for(const field of schema){
   ...
   if(['refs','claims','sources'].includes(field.type)) value=data.getAll(field.key);
   else if(field.type==='checkbox') value=value==='on';
   else if(field.type==='lines') value=['regions','country_codes','actors','keywords','exclude_keywords','languages','config.countries','config.indicators','config.topic_ids'].includes(field.key)?splitLines(value):String(value||'').split('\n').map(v=>v.trim()).filter(Boolean);
...
export function renderPathEdge(edge,i,nodes,evidence){return `<div class="repeat-row path-edge-edit"><div class="repeat-row-head"><strong>关系 ${i+1}</strong><button type="button" data-action="remove-row" class="small ghost">移除</button></div><div class="form-grid"><div class="field"><label>起点</label><select data-edge-from>${optionHtml(nodes.map(n=>[n.id,n.label||n.id]),edge.from)}</select></div><div class="field"><label>终点</label><select data-edge-to>${optionHtml(nodes.map(n=>[n.id,n.label||n.id]),edge.to)}</select></div><div class="field full"><label>关系状态</label><select data-edge-status>${optionHtml([['unverified','待验证'],['assumption','有证据支持的分析假设'],['observed','已观察到的关联']],edge.status)}</select></div>${renderField({key:'edge-evidence-'+i,title:'证据版本',type:'refs',full:true},{['edge-evidence-'+i]:edge.evidence_version_ids||[]},{evidence})}<div class="field full"><label class="check-field"><input type="checkbox" data-edge-needs-review ${edge.needs_review?'checked':''}>此段关系仍需重新审阅</label></div><div class="field full"><label>关系说明与口径差异</label><textarea data-edge-note>${esc(edge.note||'')}</textarea></div></div></div>`;}
export function readPath(form){return {nodes:[...form.querySelectorAll('.path-node')].map(n=>({id:n.dataset.nodeId,label:n.querySelector('[data-node-label]').value.trim()})),edges:[...form.querySelectorAll('.path-edge-edit')].map(e=>({from:e.querySelector('[data-edge-from]').value,to:e.querySelector('[data-edge-to]').value,status:e.querySelector('[data-edge-status]').value,evidence_version_ids:[...e.querySelectorAll('.reference-control input[type=checkbox]:checked')].map(n=>n.value),note:e.querySelector('[data-edge-note]').value.trim(),needs_review:e.querySelector('[data-edge-needs-review]').checked}))};}
