...
| evidence / evidence | knowledge | source_id, url, source_record_id, title, excerpt, language, publisher, author, material_type, published_at, source_updated_at, collected_at, discovered_at, status, origin_evidence_id, channels[], content_fingerprint, rights, topic_id; current reads add last_collected_at and acquisition_receipt without changing Evidence history |
| evidence_acquisition / internal only | knowledge | evidence_id, evidence_version_id, source_id/source_version, job_id/job_version, collected_at, received_at, is_fixture; one current receipt per Evidence with immutable receipt versions, no body fields and no outbox event |
...
- `GET /api/evidence?...&time_field=last_collected_at&since=...&until=...` filters by the latest successful collector receipt. It does not redefine publication, discovery, event or Evidence-version time. Fixed history responses do not attach a later receipt.
...
M2 collector `knowledge.ingest(..., origin='collector', acquisition_job_id=..., acquired_at=...)` may return a content-omitting result containing identity, version, duplicate status and metadata. A successful collector ingestion atomically updates the separate `evidence_acquisition` receipt even when exact dedup leaves the Evidence version unchanged. Failed/retried requests, 304 responses without ingested material, deleted-record suppression and manual writes do not advance the receipt. The receipt means locally received, not newly discovered, factually changed, licensed for body storage or a renewed retention anchor. Human writes and read APIs return policy-filtered records. Title similarity is candidate-only: user confirmation updates `origin_evidence_id` and immutable history, never merges events or asserts independent confirmation.
